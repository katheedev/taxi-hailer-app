...
public class TripResponseEventConsumer {
    ...
    @Autowired
    public TripResponseEventConsumer(TripService tripService) {
        this.tripService = tripService;
    }

    @KafkaListener(topics = "${spring.kafka.order.topic.trip-response}", containerFactory="TripResponseContainerFactory")
    public void createOrderListener(@Payload TripResponseReqDto tripResponseReqDto, Acknowledgment ack) {
        ...
        log.info("Notification service received order {} ", tripResponseReqDto.toString());
        tripService.handleTripResponse(tripResponseReqDto);
        ack.acknowledge();
    }
